...
@testset "Initiating " begin
    @testset "Simple binding sites" begin
        ...
        pop = binding_sites(N=4, l=10, n=4)
        initiate!(pop, 3)
        ...
        @test_logs (:warn, "l>L. Choosing L=l.") binding_sites(N=4, l=10, L=2, n=4)
        @test_nowarn initiate!(pop, overwrite=true)
    ...
    @testset "Driver Trailer sites" begin
        @testset "Random sequences" begin
            ...
            pop1 = driver_trailer(N=4, l=10, n=4, m=4)
            initiate!(pop1, 3)
            # Test population size
            ...
            @test_nowarn initiate!(pop1, overwrite=true)

        end

        @testset "Optimal Sequence" begin

            pop = driver_trailer(N=4, l=10, n=4, m=4)
            initiate!(pop, 3, opt=true)
        ...
        @testset "Given Driver" begin
            ...
            pop2 = driver_trailer(N=4, l=10, n=4, m=4)
            initiate!(pop2, 3, driver=[1, 2, 3, 4])
            # Test that partial driver sequence is incorporated
            @test pop2.driver[1:4] == [1, 2, 3, 4]
            pop3 = driver_trailer(N=4, L=10, n=4, m=12)
            initiate!(pop3, 3, driver=collect(1:12))
            # Test that driver too long is cut short
            @test pop3.driver == collect(1:10)
            # Test warning message
            @test_logs (:warn, "l>L. Choosing L=l.") driver_trailer(N=4, l=10, L=2, n=4)

            pop4 = driver_trailer(N=4, L=10, n=4, m=4)
            # Test error message for wrong driver sequence
            @test_throws ArgumentError initiate!(pop4, 3, driver=collect(1:12))
    ...
    @testset "Driver Trailer sites variable length" begin
        @testset "Random Sequences" begin
            ...
            pop1 = driver_trailer_l(N=4, l_0=10, n=4, m=4, L=20)
            initiate!(pop1, 3)
            # Test population size
            @test sum(pop1.freqs) == pop1.N
            # Test number of species
        ...
        @testset "Optimal Sequence" begin
            ...
            pop = driver_trailer_l(N=4, l_0=10, n=4, m=4, L=20)
            initiate!(pop, 3, opt=true)
            @test length(pop.seqs) == 1
            @test pop.seqs[1] == pop.driver
            @test length(pop.freqs) == 1
        ...
        @testset "Given Trailer" begin
            pop2 = driver_trailer_l(N=4, l_0=10, n=4, m=4)
            initiate!(pop2, 3, driver=[1, 2, 3, 4])
            # Test that partial driver sequence is incorporated
            @test pop2.driver[1:4] == [1, 2, 3, 4]

            pop3 = driver_trailer_l(N=4, L=10, n=4, m=12)
            initiate!(pop3, 3, driver=collect(1:12))
            # Test that driver too long is cut short
            @test pop3.driver == collect(1:10)
            # Test warning message
            @test_logs (:warn, "l_0>L. Choosing L=l_0.") driver_trailer_l(N=4, l_0=10, L=2, n=4)

            pop4 = driver_trailer_l(N=4, L=10, n=4, m=4)
            # Test error message for wrong driver sequence
            @test_throws ArgumentError initiate!(pop4, 3, driver=collect(1:12))
        end
    end
    ...
    @testset "Monomorphic Population" begin
        ...
        @testset "Initiate Random" begin
            pop = mono_pop(N=10, l=10, n=4, m=4)
            initiate!(pop)
...
@testset "Mutations" begin
    @testset "Find existing sequences" begin
        pop = binding_sites(N=8, L=1, n=4, seqs=[[1], [2], [3], [4]], freqs=[2, 2, 2, 2])
        mutation!(pop)
    ...
    @testset "Driver mutation" begin
        pop = driver_trailer(N=4, L=10, n=4, m=12)
        initiate!(pop, 1, driver=collect(1:12))
        mutation!(pop)
        # Test that new species is created
        @test length(pop.seqs) == 2
        @test length(pop.freqs) == 2
    ...
    @testset "Length mutation" begin
        pop = driver_trailer_l(N=4, L=10, n=4, m=12)
        initiate!(pop, 1, driver=collect(1:12))
        mutation!(pop)
        # Test that new species is created
        @test length(pop.seqs) == 2
        ...
        pop2 = driver_trailer_l(N=4, L=10, n=4, m=4)
        initiate!(pop2, 3)
        tmp_driver = copy(pop2.driver)
        driver_mutation!(pop2)
        # Test that driver sequence is mutated
        ...
        pop3 = driver_trailer_l(N=1, L=10, n=4, m=4)
        initiate!(pop3, 1)
        mutation!(pop3)
        # Test that new species is created
        ...
        pop4 = driver_trailer_l(N=1, L=10, n=4, m=4)
        initiate!(pop4, 1)
        length_mutation!(pop4)
        # Test that new species is created
...
@testset "Substitutions" begin
    ...
    f = fermi_fitness(l=10, beta=1, f0=1, fl=0)
    # Test error for too many species
    @test_throws ArgumentError bp_substitution!(pop, emat, f)
